...
const cartSlice = createSlice({
  ...
  reducers: {
      ...
      if (tempItem) {
        const tempCart = state.data.map((item) => {
          if (item.id == action.payload.id) {
            let newQty = state.amount + action.payload.quantity;
    ...
    clearCart(state) {
      (state.data = []), storeInLocalStorage(state.data);
    },
    getCartTotal(state) {
      state.totalAmount = state.data.reduce((cartTotal, cartItem) => {
        return (cartTotal += cartItem.totalPrice);
      }, 0);
      state.totalItems = state.data.reduce((sumItems, curItems) => {
        return (sumItems += curItems.quantity);
      }, 0);
    },
